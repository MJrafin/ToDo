...
class PostController extends Controller
{
    public function FetchPost(){
        ...
        $like_dislike_list = like_dislike_list::where('email','=', session('user')->email)->get();
        return response()->json([
            'posts' => $posts,
            'like_dislike_list' => $like_dislike_list
        ]) ;  //redirect to home with the $post oject
    }
    ...
    public function like($post_id){ 
        $user = like_dislike_list::where('post_id','=',$post_id)  
        ...
        if(isset($user)){  //check if there are any eliment found or not
            ...
            if($user->liked == 1 && $user->disliked == 0){  //if a user previously liked the post and again hit the like button.
                $post = post::where('id','=', $post_id)->decrement('number_of_like');

                $like_list = like_dislike_list::where('post_id','=',$post_id)->where('email','=', session('user')->email)->decrement('liked');
                //$post2 = post::where('id','=', $post_id)->first();
            ...
            elseif ($user->liked == 0 && $user->disliked == 1) {  //if a user previously disliked the post but again hit the like button.
                $post = post::where('id','=', $post_id)->increment('number_of_like');
                $post = post::where('id','=', $post_id)->decrement('number_of_dislike');

                $like_list = like_dislike_list::where('post_id','=',$post_id)->where('email','=', session('user')->email)->increment('liked');
                $like_list = like_dislike_list::where('post_id','=',$post_id)->where('email','=', session('user')->email)->decrement('disliked');
                //$post2 = post::where('id','=', $post_id)->first();
                // echo $post2->number_of_like;
                // return redirect('viewposts');  //redirect to home with the $post oject
            }
    ...
    public function dislike($post_id){
        ...
        if (isset($user)) {
            ...
            if($user->disliked == 1 && $user->liked == 0){
                ...
                $dislike_list = like_dislike_list::where('post_id','=',$post_id)->where('email','=', session('user')->email)->decrement('disliked');
                
               // return redirect('viewposts');
            }
            elseif ($user->disliked == 0 && $user->liked == 1) {
                $post = post::where('id','=', $post_id)->increment('number_of_dislike');
                $post = post::where('id','=', $post_id)->decrement('number_of_like');

                $dislike_list = like_dislike_list::where('post_id','=',$post_id)->where('email','=', session('user')->email)->decrement('liked');
                $dislike_list = like_dislike_list::where('post_id','=',$post_id)->where('email','=', session('user')->email)->increment('disliked');
                //return redirect('viewposts');  //redirect to home with the $post oject
            }
            else{
                $post = post::where('id','=', $post_id)->increment('number_of_dislike');
                ...
                $dislike_list = like_dislike_list::where('post_id','=',$post_id)->increment('disliked');
                //return redirect('viewposts');  //redirect to home with the $post oject
