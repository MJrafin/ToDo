...
<body style="background-color: white; width: 100%;">
        ...
        <div class="collapse navbar-collapse justify-content-end" id="navbarSupportedContent" style="color: #361d32;">
            <ul class="nav navbar-nav">
                <li class="nav-item active">
                    <a class="nav-link" href="{{ url('viewposts') }}">Home<span
                            class="sr-only">(current)</span></a>
                </li>
                @if (!session()->has('user'))
                    <!-- if there is no user loged in, register and Login button will be shown in the header-->
                    <li class="nav-item">
                ...
                @if (session()->has('user'))
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="UserDropdown" role="button"
                            ...
                            <!-- if there is any user loged in, his name will be shown -->
                        </a>
    ...
    <!-- Post Text Field -->
    <div class=" container justify-content-center mb-4 text-container"
        style="align-items: center; padding-bottom: 50px;">

        <h5 class="pt-5" style="padding-left: 7px;">Write post</h5>
        <button type="button" class="btn btn-light large-btn" data-toggle="modal" data-target="#PostModal"
            data-whatever="@mdo">What's on your mind?</button>

        <div class="post-btn" style="float: left;">
            <button type="button" class="btn mt-2 btn-primary btn-sm" data-toggle="modal"
                data-target="#PostModal">Post</button>
        </div>
                    ...
                    <div class="modal-body">
                        <form action="{{ url('dopost') }}" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="content" class="form-control post-text" id="message-text"
                                    rows="2" cols="2" placeholder="What's on your mind?"></input>
                            </div>
   ...
   <div class=" container justify-content-center pt-2 pb-2" style="align-items: center;">
    <div class="card">
      ...
      <!-- <form action="{{url('like')}}" method="post">
        @csrf -->
          <button  class="like_button{{$post->id}} like-btn" id="like_button{{$post->id}}">
...
<script>
  ...
  @endforeach --}}
    ...
    <!-- takes the posts one by one and shows in the desired position   -->
    <div class="card_post container justify-content-center pt-2 pb-2" style="align-items: center;">
            ...
            function fetchposts() {
                $.ajax({
                        ...
                        success: function(response) {
                            ...
                            //  console.log(response.posts);
                            $('.card_post').html("");
                            $.each(response.posts, function(post_indexInArray, post_valueOfElement) {

                                    if (!jQuery.isEmptyObject(response.like_dislike_list)) {

                                        $.each(response.like_dislike_list, function(ldl_indexInArray, ldl_valueOfElement) {

                                            if (ldl_valueOfElement.post_id === post_valueOfElement.id) {

                                                if (ldl_valueOfElement.liked === 1) {
                                                    temp = 1;
                                                    Lcolor = "blue";
                                                } else if (ldl_valueOfElement.disliked === 1) {
                                                    temp = 1;
                                                    Dcolor = "red";
                                                } else if (ldl_valueOfElement.liked === 0 && ldl_valueOfElement.disliked === 0) {
                                                    temp = 1
                                                }
                                            }
                                        });
                                        if (temp === 1) {
                                          $('.card_post').append('<div class="card">\
                                          <div class="card-body">\
                                          <h5 class="card-title">'+post_valueOfElement.name+'</h5>\
                                            <p class="card-text">\
                                            <small class="text-muted">'+HumanRead(post_valueOfElement.created_at)+'</small>\
                                            </p>\
                                            <p class="card-text">'+post_valueOfElement.content+'</p>\
                                          <div class="card-footer bg-white post-options">\
                                            <button  class="like_button like-btn" id="like_button'+post_valueOfElement.id+'" value="'+post_valueOfElement.id+'" style="color:'+Lcolor+';">\
                                            <i class="fas fa-thumbs-up pt-1" ><span style=" padding-left: 2px; padding-right: 10px">'+post_valueOfElement.number_of_like+' Like</span>\
                                            </i>\
                                            </button>\
                                            <button tupe="submit" class="dislike_button dislike-btn" id="dislike_button'+post_valueOfElement.id+'" value="'+post_valueOfElement.id+'" style="color:'+Dcolor+';">\
                                            <i class="fas fa-thumbs-down pt-1" ><span style="padding-left: 2px; padding-right: 10px">'+post_valueOfElement.number_of_dislike+' Dislike</span></i>\
                                            </button>\
                                            <div class="share-btn">\
                                            <i class="fas fa-share"></i><span> Share</span>\
                                            </div>\
